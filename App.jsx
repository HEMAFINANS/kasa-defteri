...
const AY_ADI_UZUN = ["Ocak", "Şubat", "Mart", "Nisan", "Mayıs", "Haziran", "Temmuz", "Ağustos", "Eylül", "Ekim", "Kasım", "Aralık"];
...
export default function KasaDefteri() {
  ...
  const [loanDraft, setLoanDraft] = useState({ name: "", monthly: "", start: todayMK(), months: "", dueDay: "" });
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        setData(res ? { ...emptyData(), ...JSON.parse(res.value) } : emptyData());
      } catch (e) {
        setData(emptyData());
      } finally {
        setLoading(false);
  ...
  const forecast = useMemo(() => {
    ...
    for (let i = 0; i < HORIZON; i++) {
      ...
      let expense = 0;
      let breakdown = { recurring: 0, installments: 0, loans: 0, cards: 0, vendors: 0 };
      ...
      data.loans.forEach((loan) => {
        const endIdx = mkToIndex(loan.start) + loan.months - 1;
        if (mkToIndex(mk) >= mkToIndex(loan.start) && mkToIndex(mk) <= endIdx) { expense += loan.monthly; breakdown.loans += loan.monthly; }
      });
  ...
  const addLoan = () => {
    if (!loanDraft.name.trim() || !loanDraft.monthly || !loanDraft.months) return;
    persist({
      ...data,
      loans: [
        ...data.loans,
        { id: uid(), name: loanDraft.name.trim(), monthly: Number(loanDraft.monthly), start: loanDraft.start, months: Number(loanDraft.months), dueDay: Number(loanDraft.dueDay) || null },
      ],
    });
    setLoanDraft({ name: "", monthly: "", start: todayMK(), months: "", dueDay: "" });
    setOpenForm(null);
  };
  const removeLoan = (id) => persist({ ...data, loans: data.loans.filter((l) => l.id !== id) });
  ...
  const totalLoanRemaining = data.loans.reduce((s, loan) => {
    const endIdx = mkToIndex(loan.start) + loan.months - 1;
    const remaining = Math.max(0, Math.min(loan.months, endIdx - mkToIndex(todayMK()) + 1));
    return s + remaining * loan.monthly;
  }, 0);
  const totalDebtLoad = totalCardDebt + totalVendorDebt + totalInstallmentRemaining + totalLoanRemaining;

  // Upcoming loan payments by exact day (for reminders)
  const nowDate = new Date();
  const upcomingLoanPayments = data.loans
    .filter((l) => l.dueDay)
    .map((l) => {
      const startIdx = mkToIndex(l.start);
      const endIdx = startIdx + l.months - 1;
      let candidate = new Date(nowDate.getFullYear(), nowDate.getMonth(), l.dueDay);
      if (candidate < new Date(nowDate.getFullYear(), nowDate.getMonth(), nowDate.getDate())) {
        candidate = new Date(nowDate.getFullYear(), nowDate.getMonth() + 1, l.dueDay);
      }
      const candidateMk = `${candidate.getFullYear()}-${pad(candidate.getMonth() + 1)}`;
      const candidateIdx = mkToIndex(candidateMk);
      if (candidateIdx < startIdx || candidateIdx > endIdx) return null;
      const daysUntil = Math.round((candidate - new Date(nowDate.getFullYear(), nowDate.getMonth(), nowDate.getDate())) / 86400000);
      return { ...l, nextDue: candidate, daysUntil };
    })
    .filter(Boolean)
    .sort((a, b) => a.nextDue - b.nextDue);

  // Bank loans monthly breakdown
  const loanBreakdown = (() => {
    if (data.loans.length === 0) return [];
    const startIdx = mkToIndex(todayMK());
    const maxEndIdx = Math.max(...data.loans.map((l) => mkToIndex(l.start) + l.months - 1));
    let remaining = totalLoanRemaining;
    const rows = [];
    for (let idx = startIdx; idx <= maxEndIdx; idx++) {
      const mk = indexToMk(idx);
      const items = data.loans
        .filter((l) => idx >= mkToIndex(l.start) && idx <= mkToIndex(l.start) + l.months - 1)
        .map((l) => ({ name: l.name, amount: l.monthly, dueDay: l.dueDay }));
      const monthTotal = items.reduce((s, it) => s + it.amount, 0);
      remaining -= monthTotal;
      rows.push({ mk, total: monthTotal, remainingAfter: Math.max(0, remaining), items });
    }
    return rows;
  })();
    ...
    >
      <div className="max-w-6xl mx-auto px-5 py-8">
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 gap-3 mb-8">
          <SummaryCard label="Toplam Banka Kredisi Borcu" value={formatTL(totalLoanRemaining)} accent="#A13D2B" />
        ...
        {upcomingLoanPayments.length > 0 && (
          <section className="mb-10 rounded-lg border-2 p-4" style={{ borderColor: "#1B3630", background: "#F5F0E4" }}>
            <h2 className="flex items-center gap-2 text-lg font-semibold mb-3" style={{ color: "#1B3630", fontFamily: "ui-serif, Georgia, serif" }}>
              <AlertTriangle size={20} /> Yaklaşan Kredi Ödemeleri
            </h2>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
              {upcomingLoanPayments.map((l) => {
                const urgent = l.daysUntil <= 3;
                const soon = l.daysUntil <= 7;
                return (
                  <div
                    key={l.id}
                    className="rounded-lg border p-3 flex items-center justify-between"
                    style={{ borderColor: urgent ? "#A13D2B" : "#1B3630", background: urgent ? "#A13D2B18" : soon ? "#A8763E18" : "white" }}
                  >
                    <div>
                      <div className="font-semibold">{l.name}</div>
                      <div className="text-xs opacity-70">
                        {l.nextDue.getDate()} {AY_ADI_UZUN[l.nextDue.getMonth()]} · {formatTL(l.monthly)}
                      </div>
                    </div>
                    <div className="text-sm font-semibold" style={{ color: urgent ? "#A13D2B" : soon ? "#A8763E" : "#1B3630" }}>
                      {l.daysUntil === 0 ? "Bugün" : l.daysUntil === 1 ? "Yarın" : `${l.daysUntil} gün sonra`}
                    </div>
                  </div>
                );
              })}
        ...
        {/* Bank loans */}
        <Section icon={<Landmark size={20} />} title="Banka Kredileri" onAdd={() => setOpenForm(openForm === "loan" ? null : "loan")} addLabel="Kredi Ekle">
          {openForm === "loan" && (
            <FormRow>
              <input value={loanDraft.name} onChange={(e) => setLoanDraft({ ...loanDraft, name: e.target.value })} placeholder="Kredi adı (örn. QNB İhtiyaç Kredisi)" className="input flex-1 min-w-[140px]" />
              <input value={loanDraft.monthly} onChange={(e) => setLoanDraft({ ...loanDraft, monthly: e.target.value })} placeholder="Aylık taksit" type="number" className="input w-32" />
              <input value={loanDraft.months} onChange={(e) => setLoanDraft({ ...loanDraft, months: e.target.value })} placeholder="Kaç ay" type="number" className="input w-24" />
              <input value={loanDraft.start} onChange={(e) => setLoanDraft({ ...loanDraft, start: e.target.value })} type="month" className="input w-36" />
              <input value={loanDraft.dueDay} onChange={(e) => setLoanDraft({ ...loanDraft, dueDay: e.target.value })} placeholder="Ödeme günü (1-31)" type="number" min="1" max="31" className="input w-32" />
              <button onClick={addLoan} className="btn-primary">Ekle</button>
            </FormRow>
          )}
          {data.loans.length === 0 ? (
            <Empty text="Henüz banka kredisi eklenmedi." />
          ) : (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
              {data.loans.map((loan) => {
                const endMk = addMonths(loan.start, loan.months - 1);
                const remaining = Math.max(0, Math.min(loan.months, mkToIndex(endMk) - mkToIndex(todayMK()) + 1));
                return (
                  <Card key={loan.id} onDelete={() => removeLoan(loan.id)}>
                    <div className="font-semibold">{loan.name}</div>
                    <div style={{ fontFamily: "ui-monospace, monospace" }} className="text-sm">{formatTL(loan.monthly)} / ay{loan.dueDay ? ` · her ayın ${loan.dueDay}'i` : ""}</div>
                    <div className="text-xs opacity-70 mt-1">
                      {labelMk(loan.start)} → {labelMk(endMk)} · kalan {remaining} taksit
                    </div>
                  </Card>
                );
              })}
          ...
          {loanBreakdown.length > 0 && (
            <div className="mt-5">
              <div className="text-sm font-semibold mb-2" style={{ fontFamily: "ui-serif, Georgia, serif" }}>Aylara Göre Dağılım</div>
              <div className="overflow-x-auto rounded-lg border" style={{ borderColor: "#1B3630" }}>
                <table className="w-full text-sm">
                  <thead>
                    <tr style={{ background: "#1B3630", color: "white" }}>
                      <th className="text-left px-3 py-2">Ay</th>
                      <th className="text-left px-3 py-2">O ay ödenecek</th>
                      <th className="text-left px-3 py-2">Kalan toplam</th>
                    </tr>
                  </thead>
                  <tbody>
                    {loanBreakdown.map((row, i) => (
                      <tr key={row.mk} style={{ background: i % 2 === 0 ? "#F5F0E4" : "#EDE6D6" }}>
                        <td className="px-3 py-2">{labelMk(row.mk)}{row.mk === todayMK() ? " (bu ay)" : ""}</td>
                        <td className="px-3 py-2" style={{ fontFamily: "ui-monospace, monospace" }}>
                          {formatTL(row.total)}
                          {row.items.length > 0 && (
                            <div className="text-xs opacity-60 font-sans">
                              {row.items.map((it) => `${it.name}${it.dueDay ? ` (${it.dueDay}'i)` : ""}`).join(", ")}
                            </div>
                          )}
                        </td>
                        <td className="px-3 py-2" style={{ fontFamily: "ui-monospace, monospace" }}>{formatTL(row.remainingAfter)}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
